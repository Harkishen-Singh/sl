...
function campaigns_add(req,res){
	...
	font varchar(20) null,background varchar(20) null);'
		conn.query(q, (e)=>{
	...
	q='insert into campaigns(name,short_description,offer_code,expiry,offer_description,long_description,image_address,font,background) values \
		("'+name+'","'+shortdescription+'","'+offer+'","'+expirydate.substr(0,25)+'","'+offerdescription+'","'+longdescription +'","'+img_add+'","'+
		font+'","'+back+
		'");';
	console.warn(q)
	img_saved=false;
	conn.query(q, (e)=>{if (e) {console.log('[ERR] err while inserting into campaigns table. same name might already exist or something else.');isErr=true;}
