...
function add_agentComission(req,res) {
	...
	console.log(name+currency+country+fixedo+type+percentageo+validity+minsignups+slots);
		...
		q='create table agents_comission(id int primary key auto_increment, name varchar(100) null, currency varchar(100) null, \
		 country varchar(100) null,type varchar(20) null, fixed_amount float null, percent float null,\
	...
	q= 'insert into agents_comission(name,currency,country,type,fixed_amount,\
		percent,validity,minsignups,slots, slot_duration) values ("'+name+'","'+currency+'","'+country+'","'+type+'",'+fixedo+','+percentageo+','+validity
		...
		if(e){console.error('[ERR while retriving data from agents_comission table');isErr=true;throw e}
			...
			for(let rr=0; rr< r.length;rr++) {
				obj = {
					'id':r[rr]['id'],
					'name':r[rr]['name'],
					'currency':r[rr]['curency'],
					'country':r[rr]['country'],
					'type':r[rr]['type'],
					'fixed_amount':r[rr]['fixed_amount'],
					'percent':r[rr]['percent'],
